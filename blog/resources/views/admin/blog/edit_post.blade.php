    ...
    <div class="container">
        ...
        <form action="{{route('admin.blog.post.update')}}" method="post">
            ...
            <div class="input-group">
                ...
                <textarea name="body" id="body" rows="9" {{$errors->has('body') ? 'class=has-error' : ''}}>{{Request::old('body') ? Request::old('body') : isset($post) ? $post->body : ''}}</textarea>
            </div>
...
@endsection
@section('scripts')
    <script type="text/javascript" src="{{asset('src/js/posts.js')}}"></script>
@endsection
